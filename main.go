...
func main() {
	...
	fmt.Printf("Done. OK: %d  FAIL: %d  Matches: %d\n", ok, fail, matches)
	if firstIdx != -1 {
...
func fetch(ctx context.Context, client *http.Client, rawURL string, readCapMB int, ua string) ([]byte, string, error) {
	...
	// Let net/http handle gzip automatically; no need to force br/deflate
...
func parseFlags() Config {
	...
	flag.IntVar(&cfg.Concurrency, "concurrency", 8, "Number of parallel workers")
